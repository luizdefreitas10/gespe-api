...
export class ExcelReaderService {
  ...
  getCellValue(sheet: XLSX.WorkSheet, cellAddress: string): any {
    const cell = sheet[cellAddress]
    return cell?.v || null
  }

  getArrayCellValue(row: any[], index?: number): string | null {
  ...
  parseExcelDate(dateValue: any): Date {
    ...
    if (typeof dateValue === 'number') {
      if (dateValue < 0 || dateValue > 100000) {
        console.warn(`⚠️ Data Excel inválida (número muito grande): ${dateValue}`)
        return new Date()
      }
      ...
      if (year < 1900 || year > 2100) {
        console.warn(`⚠️ Data resultante fora do range: ${jsDate.toISOString()} (valor Excel: ${dateValue})`)
        return new Date()
      }
    ...
    console.warn(`⚠️ Não foi possível parsear data: ${dateValue} (tipo: ${typeof dateValue})`)
    return new Date()
  }
